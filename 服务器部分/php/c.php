...
if($method == 'common_detail'){
    $biao = $post['biao'];
    $key = $post['tj'] == null ? 'id' : $post['tj'];
    echo chaxun_one($biao,array('tj'=>$key));
}
...
if($method == 'chat_getchat'){
    ...
    for($i=0;$i<$count;$i++){
        ...
        $duifang = $conn->query("SELECT wxhead,wxname FROM user where id = $dfid")->fetch_assoc();
        $list[$i]['wxname'] = $duifang['wxname'];
        $list[$i]['wxhead'] = $duifang['wxhead'];
        $list[$i]['dfid'] = $dfid;
    }
...
if($method == 'chat_getlist'){
    ...
    if($code ->num_rows==0){
        ...
        $sql = "SELECT u.wxname,u.wxhead,c.img,c.neirong,c.uid1,c.uid2,c.code,c.id,c.addtime FROM $biao_user u,$biao_chat c where c.uid1 = u.id and c.code = $c order by c.id asc";
        echo chaxun($sql);
    }
...
if($method == 'chat_getnew'){
    $code = $post['code'];
    $skip = $post['skip'];
    $sql  = "SELECT u.wxname,u.wxhead,c.img,c.neirong,c.addtime,c.uid1,c.uid2,c.id FROM $biao_user u,$biao_chat c where c.uid1 = u.id and c.code = '$code' and c.id > $skip";
    echo chaxun($sql);
}

if($method == 'chat_getname'){
    $id = $post['id'];
    $name = $conn->query("SELECT wxname FROM $biao_user where id = $id")->fetch_assoc()['wxname'];
